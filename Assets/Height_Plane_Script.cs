        ...
        foreach (GameObject go in gos) {
        	...
        	if ( distance < 40)
        	{
	        	GameObject highlight = Instantiate(Resources.Load("Block_Highlight", typeof(GameObject))) as GameObject;
	       		highlight.transform.position = new Vector3(go.transform.position.x, player.transform.position.y+ 0.15f, go.transform.position.z);
	       		highlight.transform.localScale = new Vector3( go.transform.lossyScale.x * 0.4588511469f * 0.2f + 0.1f, 0.1f, go.transform.lossyScale.z * 0.4588511469f  * 0.2f+ 0.1f);
	      		...
	      		//print(height_color);
	       		highlight.renderer.material.color = new Color (0.5f, 0, 0);
       		}
